...
def f(a1:float, a2:float, min_v:int, min_s:int, n_init:int) -> Dict[str, float]:
    cmd = [CMD, str(a1), str(a2), str(min_v), str(min_s), str(n_init)]
...
def calc(alpha1: List[float], alpha2: List[float], min_v:List[int], min_s:List[int], n_init:List[int]):
    # Vars for finding max
    indices = [0, 0, 0, 0, 0]
    val_max = 0

    alphas_combinations = itertools.product(alpha1, alpha2, min_v, min_s, n_init)
    ...
    for coefs in alphas_combinations:
        ...
        while result is None:
            result = f(coefs[0], coefs[1], coefs[2], coefs[3], coefs[4])
        ...
        if result[TO_OPTIMIZE] > val_max:
            ...
            indices[3] = n_init.index(coefs[3])
            print('\t Actual Max value: {:.4f} % -> a1: {:.4f}, a2: {:.4f}, minV: {}, minS: {}, nInit: {}'.format(val_max, coefs[0], coefs[1], coefs[2], coefs[3], coefs[4]))
    ...
    print('Max value: {:.4f} % -> a1: {:.4f}, a2: {:.4f}, minV: {:.4f}, minS: {:.4f}, nInit: {:.4f}'.format(val_max,
            alpha1[indices[0]],
            alpha2[indices[1]],
            min_v[indices[2]],
            min_s[indices[3]],
...
if __name__ == '__main__':
    ...
    min_v = list(np.linspace(0, 10, num=N_SAMPLES))
    min_s = list(np.linspace(0, 10, num=N_SAMPLES))
    n_init = list(np.linspace(0, 10, num=N_SAMPLES))
    ...
    for i in range(nIters):
        ...
        alphas_range = calc(alpha1, alpha2, min_v, min_s, n_init)
        ...
        n_init = list(np.linspace((alphas_range[3][0]), (alphas_range[3][1]), num=N_SAMPLES))
